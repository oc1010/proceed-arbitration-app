...
# --- DATA LOAD & REPAIR ---
# Immediate repair to prevent crashes
...
def get_party_emails():
    ...
    # Return list including duplicates if any (e.g. testing)
    emails = []
...
with tab1:
    if not timeline:
        ...
        # TABLE (READ ONLY - Prevents Refresh Issues)
                ...
                with st.container(border=True):
                    ...
                    if evt_opts:
                        ...
                        with st.form("edit_form"):
                            ...
                            # EXPANDABLE TEXT AREA for To-Do
                            new_log = st.text_area("To-Do / Logistics", value=c_log, height=150, help="Drag corner to expand")
                    ...
                    with st.form("add_form"):
                        ...
                        if st.form_submit_button("Add Event"):
                            new_e = {
                                ...
                                "logistics": n_log, "status": "Upcoming", "history": ["Added manually"]
                            }
    ...
    if role in ['claimant', 'respondent']:
        with st.form("eot_req"):
            ...
            if st.form_submit_button("Submit Request"):
                delays.append({
                    "event": tgt, "requestor": role, "reason": rsn, 
                    "proposed_date": str(prop), "status": "Pending", "tribunal_decision": ""
                })
                save_complex_data("delays", delays)
                # Notify Arbitrator (simulated via toast for now or email if needed)
    ...
    if delays:
        for i, d in enumerate(delays):
            with st.container(border=True):
                ...
                if d.get('tribunal_decision'):
                    c2.info(f"Note: {d['tribunal_decision']}")
